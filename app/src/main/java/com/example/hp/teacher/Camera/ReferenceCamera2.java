...
public class ReferenceCamera2 extends ReferenceCamera {
    ...
    public ReferenceCamera2(Context context, int deviceRotation) {
    ...
    public ReferenceCamera2(Context context, int deviceRotation, int ratio) {
        mDeviceRotation = deviceRotation;
        mIsStarted = new AtomicBoolean(false);
    ...
    private Size getOptimalSize(Size[] sizes, int w, int h, String where) {
        ...
        for (Size size : sizes) {
            ...
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.N) {
                ...
                isCorrectkSize = (maxSize <= 1280 && size.getHeight() <= w);
            }
    ...
    private Size getFittedPreviewSize(Size[] supportedPreviewSize, float previewRatio) {
        ...
        for (Size size : supportedPreviewSize) {
            ...
            if (Math.abs(ratio - previewRatio) < EPSILON && max(size.getWidth(), size.getHeight()) <= 1024 ) {
        ...
        public void onImageAvailable(final ImageReader reader) {
            mHandler.post(new Runnable() {
                ...
                public void run() {
                    ...
                    if (image != null) {
                        boolean ret = ARGearSDK.processFrame(image);
                        if (mListener != null) {
        ...
        private void process(CaptureResult result) {
            ...
            if(faces != null && mode != null) {
                if (faces.length > 0) {
                    ...
                    int MAXFACE = 5;
                    if (numFaces>MAXFACE) numFaces = MAXFACE;
                     ...
                     */
                    ARGearSDK.setFaceRects2(numFaces, bbox);
                }
            }
        }
